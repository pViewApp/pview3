...
pvui::MainWindow::MainWindow(QWidget *parent)
    : QMainWindow(parent), navigationWidget(new QTreeView),
      navigationModel(new QStandardItemModel), content(new QWidget),
...
void pvui::MainWindow::setupMenuBar() {
  ...
  {
    auto *newItem = new QAction("&New");
    QObject::connect(newItem, &QAction::triggered, this,
...
void pvui::MainWindow::setupNavigation() {
  ...
  QItemSelectionModel *selectionModel =
      new QItemSelectionModel(navigationModel);
...
void pvui::MainWindow::setupDataFile() {
  dataFile().accountAdded().connect([&](pv::AccountPtr account) {
    ...
    auto *navigationItem =
        new QStandardItem(QString::fromStdString(account->name()));
...
void pvui::MainWindow::addAccount() {
  bool ok = false;
  QString text =
      QInputDialog::getText(this, tr("Create an Account"), tr("Account Name:"),
                            QLineEdit::Normal, "", &ok);
  ...
  if (ok) {
    ...
    if (!trimmedText.isEmpty()) {
      ...
      QMessageBox::warning(this, tr("Invalid Account Name"),
                           tr("The account name must not be empty"));
    }
  }
}
...
void pvui::MainWindow::pageChanged(const QItemSelection &selection) {
  ...
  if (accountNavigationItems.find(selectedItem) !=
      accountNavigationItems.cend()) {
