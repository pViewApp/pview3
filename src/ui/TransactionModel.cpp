...
pvui::models::TransactionModel::TransactionModel(const pv::AccountPtr account,
                                                 QObject *parent)
    : QAbstractItemModel(parent), account_(account) {
  // Listen for before transaction added

  QObject::connect(this, &TransactionModel::beforeTransactionAdded, this,
                   &TransactionModel::beginInsertRows);
  ...
  auto beforeTransactionAddedConnection =
      account_->beforeTransactionAdded().connect(beforeTransactionAddedSlot);
  ...
  QObject::connect(this, &TransactionModel::afterTransactionAdded, this,
                   &TransactionModel::endInsertRows);

  auto afterTransactionAddedSlot = [&](const pv::TransactionPtr) {
    emit this->afterTransactionAdded();
  };

  auto afterTransactionAddedConnection =
      account_->transactionAdded().connect(afterTransactionAddedSlot);

  QObject::connect(this, &QObject::destroyed, this,
                   [&]() { afterTransactionAddedConnection.disconnect(); });
}

QVariant pvui::models::TransactionModel::data(const QModelIndex &index,
                                              int role) const {
  ...
  pv::Transaction &transaction =
      *static_cast<pv::Transaction *>(index.internalPointer());
  switch (index.column()) {
  case 0: {
...
QVariant pvui::models::TransactionModel::headerData(int section,
